...
import useDeleteListing from '@/hooks/listings/useDeleteLisitng'
import useEditListing from '@/hooks/listings/useEditListing'
import React, { useEffect, useState } from 'react'
import { useLocalSearchParams, useRouter } from 'expo-router'
import { ChevronLeft, Delete, Edit2, PlusCircle, Trash, Trash2, X } from 'lucide-react-native'
...
export default function edit() {
    ...
    const {getListing,loading:loadingGet} = useGetListing()
    ...
    useEffect(() => {
        const getCurrentListing = async () => {
            ...
            const formattedData = {
                ...
                price_onsale: data.price_onsale?.toString() || '',
                ...
                heating_type: data.heating_type || 'none',
            }
            setImages(formattedData.images)
            console.log(formattedData,34);
...
useEffect(() => {
    ...
    if (listingData?.country && !loadingGet) {
        fetchCities();
    }
}, [listingData?.country, loadingGet]); // Only run when country changes
    ...
    const {deleteListing,success:successDelete,loading:loadingDelete} = useDeleteListing()
    ...
    const { editListing,success, loading: loadingListing, error: listingError } = useEditListing()

    const handleEdit = async () => {
        const formData = new FormData()
        for (const key in listingData) {
            if(!listingData[key] || listingData[key] == '' ){
                delete listingData[key]
            }            
        }
        console.log(listingData,3445);
        
        Object.keys(listingData).forEach((key) => formData.append(key, listingData[key]))
        const imagesToKeep = images.filter((img) => listingData?.images?.some((x) => x.id == img.id)).map((x) => x.id)
        console.log(imagesToKeep,'iamges to keep');
        const filteredImages = images.filter((f) => !f.id)
        console.log(filteredImages,345);
        formData.append('images_to_keep',imagesToKeep)
    ...
    return (
        <SafeAreaView style={{ flex: 1 }}>
            <ScrollView style={{ flex: 1, paddingHorizontal: 20 }} showsVerticalScrollIndicator={false}>
                    ...
                    <ScrollView showsHorizontalScrollIndicator={false} horizontal contentContainerStyle={{ width: '100%', alignItems: 'center', flexDirection: 'row', gap: 10 }}>
                        {images?.length > 0 && images?.map((img, index) => (
                            <View style={{ width: 90, height: 90, position: "relative" }} key={index}>
                                ...
                                <Image source={{ uri: img?.uri || img?.url }} style={{ width: 90, height: 90, borderRadius: 8 }} />
                            </View>
                        ))}
                ...
                <Button title='Edit' loading={loadingListing} onPress={handleEdit} addStyles={{ width: '100%', height: 50, backgroundColor: theme.primary }} />
            </ScrollView>
        </SafeAreaView>
    )
}
