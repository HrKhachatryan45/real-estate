...
const useGetMessages = () => {
    ...
    const getMessages = async (userId) => {
        ...
        const attempt = async (access_token) => {
                try {
                    const response = await fetch(`${HOST_URL}/api/messages/get/messages/`+userId.toString(),{
                        method: 'GET',
                        headers: {
        ...
        try{
            ...
            if (!response.ok || json.error) {
                setError(json.error || 'getMessages failed');
                setLoading(false);
                return null;    
            }
    ...
    return {getMessages,error,loading,setError,success};
}
