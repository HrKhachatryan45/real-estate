...
const useDeleteConversation = () => {
    ...
    const deleteConversation = async (convId) => {
        ...
        const attempt = async (access_token) => {
                try {
                    const response = await fetch(`${HOST_URL}/api/messages/delete/conversation/${convId}`,{
                        method: 'DELETE',
                        headers: {
        ...
        try{
            ...
            if (!response.ok || json.error) {
                setError(json.error || 'deleteConversation failed');
                setLoading(false);
                return null;    
            }
    ...
    return {deleteConversation,error,loading,setError,success};
}
