...
const useGetFavRecent = () => {
    ...
    const getFavRecent = async (type,page,limit) => {
        setLoading(true);
        const attempt = async (access_token) => {
                try {
                    const response = await fetch(`${HOST_URL}/api/listing/get/fav/recent?type=${type}&page=${page}&limit=${limit}`,{
                        method: 'GET',
                        headers: {
        ...
        try{
            ...
            if (!response.ok || json.error) {
                setError(json.error || 'getFavRecent failed');
                setLoading(false);
                return [];    
            }
    ...
    return {getFavRecent,error,loading,setError};
}

export default useGetFavRecent