...
def sendEmail(to,templateX,user,token = None,reset_link = None):

    subject = templateX.subject.format(fullname=user['fullname'])
    body = templateX.body.format(fullname=user['fullname'],verification_link=token,reset_link=reset_link,otp_code=user['otp_code'])
    email = EmailMessage(
        subject=subject,
        body=body,
